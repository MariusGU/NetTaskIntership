...
namespace NetTask
{
    public class MeetingsContainer
    {
        private List<Meeting> Meetings;
        public MeetingsContainer()
        {
        ...
        public MeetingsContainer(List<Meeting> meetings)
        {
            Meetings = meetings;
        }
        /// <summary>
        ...
        public void WriteToTheFile(string path)
        {
        ...
        public bool AddPersonToTheMeeting()
        {
            if (Meetings.Count() > 0)
            {
        ...
        public bool RemovePersonFromTheMeeting()
        {
            if (Meetings.Count() > 0)
            {
        ...
        public bool RemoveMeeting(string loggedInPerson)
        {
        ...
        public DateTime InputDate(string message, string error)
        {
            ...
            {
                //Console.WriteLine("Iput format was wrong, format should look like: 2022-04-28 19:59:59");
        ...
        public int InputIndex(string message, string errorMessage)
        {
            Console.WriteLine(message);
            int index = -1;
        ...
        public string CreateMeeting(string loggedInPerson)
        {
            ...
            int categoryIndex = InputIndex("Enter the index according to the category index listed above in the table: ",
                "Please enter a number according to the categories table listed above");
            ...
            int typeIndex = InputIndex("Enter the index according to the type listed above in the table: ",
                "Please enter a number according to the types table listed above");
            ...
            else
            {
                ...
                return "Wrong input index";
            }

            DateTime startDate = InputDate("Enter the starting date and time of the meeting, Formata example: 2022-04-28 19:20:00",
                "Iput format was wrong, format should look like: 2022-04-28 19:59:59");
            DateTime endDate = InputDate("Enter the end date and time of the meeting, Formata example: 2022-04-28 19:20:00",
                "Iput format was wrong, format should look like: 2022-04-28 19:59:59");

            if (startDate.Year == 0001)
            {
                return "Wrong date and time input";
            }
            ...
            {
                return "Meeting can not end before it started";
            }
            ...
            Types type, bool isTypeChosen, DateTime startDate, DateTime endDate, int attendees)
        {
            ...
            if(meetings.Count() > 0)
            {
                ...
                Console.WriteLine(String.Format("| {0, 10} | {1,20} | {2,12} | {3,15} | {4,10} | {5,25} | {6,25} | {7,10} ", 
                    "Name", "ResponsiblePerson", "Description", "Category", "Type", "StarDate", "EndDate", "Attendees"));
                Console.WriteLine(new String('-', 140));
                foreach(Meeting meeting in meetings)
                {
                    Console.WriteLine(String.Format("| {0, 10} | {1,20} | {2,12} | {3,15} | {4,10} | {5,25} | {6,25} | {7,10} ",
                        meeting.Name, meeting.ResponsablePerson, meeting.Description, meeting.Category, meeting.Type, meeting.StartDate, meeting.EndDate, AttendeesToString(meeting.Attendees)));
                }
                Console.WriteLine(new String('-', 140));
            }
        ...
        /// <summary>
        /// Method to put every string from the list of strings into one string
        /// </summary>
        /// <param name="attendeeds">List of strings to change into one string</param>
        /// <returns>Return string containing all the elements of the attendees list</returns>
        ...
        public string ListAllTheMeetings()
        {
            if (Meetings.Count > 0)
            {
                ...
                Console.WriteLine();
                int categoryIndex = InputIndex("Enter the index of the category for the filter, from the table above",
                    "Please enter a number according to the categories table listed above");
                Categories category = new Categories();
                ...
                ListTypes();
                Console.WriteLine();
                int typeIndex = InputIndex("Enter the index of the type for the filter, from the table above",
                    "Please enter a number according to the types table listed above");
                Types type = new Types();
                ...
                DateTime startDate;
                DateTime endDate;
                startDate = InputDate("Enter the date from which to filter", "Start date filter was not provided");
                endDate = InputDate("Enter the date to which to filter", "End date filter was not provided");

                int attendeesCount = InputIndex("Enter the minimum number of attendees for the filter", "Please enter a real number");
                if(attendeesCount == -1)
                {
                    attendeesCount = 0;
                }
